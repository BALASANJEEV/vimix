...
export const registerPartner = async (req, res) => {
  try {
    ...
    const partner = await Partner.create({ name, email, company, username, password: hashed });
...
export const loginPartner = async (req, res) => {
  try {
    ...
    const partner = await Partner.findOne({ where: { username, isActive: true } });
    ...
    const token = jwt.sign({ id: partner.id, role: 'partner', username: partner.username }, process.env.JWT_SECRET, { expiresIn: '2d' });
    res.json({ token });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
export const createPartnerProject = async (req, res) => {
  try {
    ...
    if (!clientId || !title || !service) return res.status(400).json({ message: 'clientId, title and service are required' });
    ...
    const project = await Project.create({
      ...
      activityLog: [{ id: `${Date.now()}_activity`, type: 'project_created', description: 'Project created by partner', timestamp: new Date().toISOString(), details: { createdBy: 'partner' } }],
      documents: {},
      meetings: [],
      totalPayments: 0,
      partnerId,
